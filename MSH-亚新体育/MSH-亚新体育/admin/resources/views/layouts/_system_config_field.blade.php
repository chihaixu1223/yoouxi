<form class="form-horizontal" action="{{ route('admin.systemconfigs.post_config_groups',['group_name' => $group_name]) }}" method="post">
    ...
    $site_domain_item = null;
    @endphp

    {{-- 先单独显示活动站域名 --}}
    @foreach($groups as $key => $item)
        @if($item->name == 'site_domain')
            @php
                $site_domain_item = $item;
                unset($groups[$key]);
            @endphp
                ...
                <div class="col-sm-4">
                    ...
                    <div class="mt-2">
                        <a href="javascript:void(0)" class="btn btn-info btn-sm"
                            ...
                            <i class="mdi mdi-refresh"></i> @lang('res.common.fix_url')
                        </a>
                        <p class="text-muted mt-1" style="font-size: 12px;"><i class="mdi mdi-information-outline"></i> 填入活动域名后点击修复按钮，可更新系统中所有图片地址</p>
                    </div>
                </div>
                <div class="col-sm-4">
    ...
    @php
        ...
        if($site_domain_item && $site_domain_item->type == \App\Models\SystemConfig::CONFIG_TYPE_EDITOR) {
            $keys->push($site_domain_item);
        }
        $keydesc = '';
